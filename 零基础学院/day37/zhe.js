...
function allCircle(data, step, multi, ctx) {
    for (var i = 0; i < data.length; i++ ) {
        filterdata = window.localStorage[data[i].region + data[i].product] ? JSON.parse(window.localStorage[data[i].region + data[i].product]) : data[i].sale;
        paintCircle(filterdata, step, multi, ctx, lineColor[i]);
...
function verDiv2(data) {   //day-37 task 新增部分---------------------
    var temparr2, i;
    var temparr = [];
        
    for (i = 0; i < data.length; i++) {
        temparr2 = window.localStorage[data[i].region + data[i].product] ? JSON.parse(window.localStorage[data[i].region + data[i].product]) : data[i].sale;
        temparr = temparr.concat(temparr2);
    }

    var temp = Math.max.apply(null, temparr).toString();
    var firstnum = +temp[0] + 1;
    for (i = 0; i < temp.length - 1 ; i++) {
        firstnum += '0';
    }
